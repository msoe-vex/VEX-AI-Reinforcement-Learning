...
class Robot:
    ...
    camera_rotation: Optional[float] = np.pi / 2  # Radians, None = auto based on team
    ...
    def __post_init__(self):
        ...
        # Default camera rotation: same as start_orientation unless explicitly set
        if self.camera_rotation is None:
            # store as a scalar float (radians)
            self.camera_rotation = float(self.start_orientation)
        # Camera offset relative to robot body orientation (radians).
        # This is kept so the camera can remain at a fixed angle relative to the robot.
        try:
            self.camera_rotation_offset = float(self.camera_rotation) - float(self.start_orientation)
        except Exception:
