...
from .pushback import PushBackGame, BlockStatus, LOADERS, NUM_BLOCKS_FIELD, GoalType, GOALS, ObsIndex, Actions, PARK_ZONES
...
class VexAICompGame(PushBackGame):
        ...
        if robots is None:
            robots = [
                Robot(name="red_robot_0", team=Team.RED, size=RobotSize.INCH_24, 
                      start_position=np.array([-42.0, 24.0], dtype=np.float32), length=15, width=15),
    ...
    def _can_park_in_zone(self, agent_state: Dict, park_zone_color: str) -> bool:
        """VAISC2: Only 24" robot can park, and only in its alliance zone."""
        if int(agent_state.get("robot_size", 0)) != 24:
            return False
    ...
    def compute_score(self) -> Dict[str, int]:
        ...
        # 3. Parked Robots (VAISC2: only 24" in alliance zone)
        for agent_name, agent_state in self.state["agents"].items():
            if agent_state.get("parked", False):
                if agent_state.get("robot_size") == 24 and agent_state.get("parked_zone") == agent_state.get("team"):
    ...
    def is_valid_action(self, agent: str, action: int, observation: np.ndarray) -> bool:
        ...
        agent_color = self.state["agents"][agent].get("team")
        ...
        if time_remaining <= 15 and agent_size == RobotSize.INCH_24.value:
            ...
            if not within_park_zone:
                ...
                return action == Actions.IDLE.value
        ...
        if time_remaining > 45 and action in [
